...
16. [redis — a database SBoB with exactly ONE allowed client](#redis--a-database-sbob-with-exactly-one-allowed-client)
...
# redis — a database SBoB with exactly ONE allowed client
...
```bash
# 0. build bobctl from the pkg submodule
cd pkg && go build -o bobctl . && cd ..

# 1. static contrast: is this SBoB inside the database-type envelope?
#    redis legitimately opens /etc/passwd + /etc/group → reads-host-files is an
#    ACCEPTED deviation, declared with --expect so --strict exits 0.
./pkg/bobctl contrast --profile example/redis-client/sbobs/ap-redis.yaml \
  --type database --expect reads-host-files --strict
# → coverage: 29 Separable / 0 Ambiguous / 0 Blind

# 2. live: apply the SBoBs FIRST, then the workload that carries the binding label
...
# 3. confirm node-agent bound the user-defined SBoB (and skipped learning)
...
# 4. canary attack: any non-redis exec must fire R0001 against the SBoB
kubectl exec -n redis-demo deploy/redis -c redis -- whoami
kubectl logs -n honey -l app=node-agent -c node-agent \
  | grep -F '"namespace":"redis-demo"' | grep '"RuleID":"R0001"'
# → R0001 /usr/bin/whoami — Separable: redis SBoB allows only redis-server/redis-cli
```

## How the SBoB is bound — the user-defined-profile label

The pod **label** `kubescape.io/user-defined-profile: <name>` is what binds the
SBoB (node-agent `pkg/objectcache/shared_container_data.go`,
`pkg/objectcache/containerprofilecache/containerprofilecache.go`):

* node-agent resolves the label value as the name of **both** the
  ApplicationProfile **and** the NetworkNeighborhood, in the pod's namespace —
  so `ap-redis.yaml` and `nn-redis.yaml` are both `name: redis` in `redis-demo`.
* a container with this label is **excluded from learning**
  (`pkg/containerprofilemanager/v1/lifecycle.go`): the user-defined SBoB *is*
  the policy, nothing gets overwritten.
* the SBoB objects must carry `kubescape.io/managed-by: User`,
  `kubescape.io/status: completed` and `kubescape.io/completion: complete` —
  storage's cleanup skips user-managed objects
  (storage `pkg/registry/file/cleanup.go`), so the SBoB persists.
* `kubescape.io/user-defined-network` appears alongside it in upstream test
  fixtures and we ship it for forward-compatibility, but as of current
  node-agent main the *profile* label is the one lookup key for both halves.
