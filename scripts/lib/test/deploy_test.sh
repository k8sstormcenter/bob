...
test_applies_and_waits() {
  ...
  stub_manifest "java-poc" "$PODS" "$deploy" "" ""
  ...
  assert_log_contains "rollout status deploy/chain-backend -n java-poc" "waits for backend rollout"
  assert_log_contains "rollout status deploy/chain-postgres -n java-poc" "waits for postgres rollout"
}
